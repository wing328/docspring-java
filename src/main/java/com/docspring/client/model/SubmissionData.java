...
public class SubmissionData {
  ...
  **/
  ...
  @Override
  public int hashCode() {
    return Objects.hash(css, data, dataRequests, fieldOverrides, html, metadata, password, test);
  }
  ...
  * @throws IOException if the JSON Object is invalid with respect to SubmissionData
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (SubmissionData.openapiRequiredFields.isEmpty()) {
          return;
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!SubmissionData.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SubmissionData` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      if (jsonArraydataRequests != null) {
        for (int i = 0; i < jsonArraydataRequests.size(); i++) {
          CreateSubmissionDataRequestData.validateJsonObject(jsonArraydataRequests.get(i).getAsJsonObject());
        };
      }
  }
